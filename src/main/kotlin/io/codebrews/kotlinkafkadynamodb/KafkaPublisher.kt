...
class KafkaPublisher(private val kafkaConfigProperties: KafkaConfigProperties) {
    ...
    private val senderOptions: SenderOptions<String, CreateUserRequest> = SenderOptions.create<String, CreateUserRequest>(producerProps)
    private val kafkaSender: KafkaSender<String, CreateUserRequest> = KafkaSender.create(senderOptions)
    ...
    fun publishMessage(key: String, value: CreateUserRequest): Mono<Void> {
        ...
        return kafkaSender.createOutbound()
                ...
                .then()
                .doOnSuccess {
                    logger.info("Successfully sent a CreateUserRequest message with id $key")
                }
    }
